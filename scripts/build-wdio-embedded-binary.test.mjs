// Unit tests for the pure logic in scripts/build-wdio-embedded-binary.mjs.
...
describe("resolveTargetDir", () => {
  it("uses a separate target-embedded directory, never target/ or target-wdio-plugin/", () => {
    ...
    assert.ok(!dir.includes("target-wdio-plugin"));
  });
});
...
describe("resolveBinaryPath", () => {
  it("resolves the Windows binary path with .exe", () => {
    ...
    assert.equal(
      binPath,
      join(WIN_REPO_ROOT, "target-embedded", "release", "rack-inventory-studio-desktop.exe"),
    );
  });
  ...
  it("resolves the Linux binary path without an extension", () => {
    ...
    assert.equal(
      binPath,
      join(LINUX_REPO_ROOT, "target-embedded", "release", "rack-inventory-studio-desktop"),
    );
  });
});
...
describe("isRegularTargetPath", () => {
  ...
  it("accepts a binary path under target-embedded/release", () => {
    const goodPath = join(WIN_REPO_ROOT, "target-embedded", "release", "rack-inventory-studio-desktop.exe");
  ...
  it("does not false-positive on a directory name that merely starts with 'target' (e.g. target-embedded)", () => {
    const goodPath = join(WIN_REPO_ROOT, "target-embedded", "release", "rack-inventory-studio-desktop.exe");
    assert.equal(isRegularTargetPath(goodPath, WIN_REPO_ROOT), false);
  });
});
...
describe("buildChildEnv", () => {
  it("sets a target-embedded CARGO_TARGET_DIR and nothing frontend-specific", () => {
    ...
    assert.ok(!("VITE_WDIO_PLUGIN" in env), "embedded build must not set VITE_WDIO_PLUGIN");
  });
...
describe("buildTauriArgs", () => {
  it("builds the expected pnpm tauri build argument list with the wdio-embedded feature", () => {
    const args = buildTauriArgs();
    assert.deepEqual(args, ["-C", "apps/desktop", "tauri", "build", "--no-bundle", "--features", "wdio-embedded"]);
  });

  it("does not pass a --config override (unlike the wdio-plugin variant)", () => {
    const args = buildTauriArgs();
    assert.ok(!args.includes("--config"), "wdio-embedded build must not pass --config");
  });
});
